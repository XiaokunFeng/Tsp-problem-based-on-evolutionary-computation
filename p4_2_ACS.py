...
"""
description: 使用 蚁群系统算法(ACS)来求解 中国34个城市的TSP问题
"""
import math
import random
import sys
        ...
        for i in range(self.antNumber):
            ...
            while len(left_city_table) != 0 :
                ...
                if np.random.uniform() < self.q0:
                    select_index = np.argmax(prob)
                else:
                    select_index = self.select_roulette(prob)

                cur_city0 = cur_city
                cur_city = left_city_table[select_index]
                # 局部信息素更新 cur_city0->cur_city
                self.tau[cur_city0,cur_city] = (1-self.xi)*self.tau[cur_city0,cur_city] + self.xi*self.tau0
...
def cal_tsp(iter_num):
    ...
    np.save("resource/aco_loss_2.npy", loss)
