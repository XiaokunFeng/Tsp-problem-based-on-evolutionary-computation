...
class GA(object):
    ...
    遗传算法类
    """
    def __init__(self, lifeCount=100,crossRate=0.8, mutationRate=0.05,selectRatio=0.2,cities_path="resource/china_cities.npy",initial_data_save_flag = False):
        ...
        # 保存初始化的数据
        if initial_data_save_flag:
            initial_data = []
            for live_item in self.lives:
                initial_data.append(live_item.gene)
            initial_data = np.array(initial_data)
            np.save("resource/initial_data_1.npy",initial_data)
...
def cal_tsp(iter_num):
    ...
    np.save("resource/loss_2.npy", loss)
    # 输出相关结果
