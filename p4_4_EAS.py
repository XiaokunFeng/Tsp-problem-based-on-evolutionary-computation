...
"""
description: 使用 精华蚂蚁系统算法(ACS)来求解 中国34个城市的TSP问题
"""
...
class ACO(object):
    ...
    def __init__(self, antNumber=50,cities_path="resource/china_cities.npy"):
        ...
        # 初始化相关矩阵
        self.tau = np.ones([self.cityNumber, self.cityNumber])*0.1  # 信息素矩阵
    ...
    def update_tau(self):
        ...
        """
        # 1.先按 ACO算法来更新 tau
        delta_tau = np.zeros((self.cityNumber, self.cityNumber))
        for ant_index in range(self.antNumber):
            ant_item = self.ants[ant_index]
            # 根据 蚂蚁个体 走过的路径，来更新信息素
            for city_index in range(-1, self.cityNumber - 1):
                city0 = ant_item.table[city_index]
                city1 = ant_item.table[city_index + 1]
                delta_tau[city0, city1] = delta_tau[city0, city1] + self.Q * ant_item.score

        # 计算蒸发后的 信息素
        self.tau = (1 - self.rho) * self.tau + self.rho * delta_tau
        ...
        delta_tau = self.e/best_item.score
        ...
        for city_index in range(-1,self.cityNumber-1):
            ...
            # 计算蒸发后的 信息素
            self.tau[city0,city1] = self.tau[city0,city1] + delta_tau
...
def cal_tsp(iter_num):
    ...
    np.save("resource/aco_loss_4.npy", loss)
